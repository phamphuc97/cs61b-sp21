...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        if (atLeastOneMoveExists(board) && !noMove()) {
            moveColumn();
            changed = true;
        }
    ...
    private void moveColumn() {
        for (int col = 0; col < board.size(); col++) {
            for( int row = board.size() - 1; row > 0; row--) {
                int lowerRow = row - 1;
                while (lowerRow >= 0) {
                    Tile lower = board.tile(col, lowerRow);
                    if (board.tile(col, row) == null && lower != null) {
                        board.move(col, row, lower);
                    }
                    else if (board.tile(col, row) != null && lower != null && board.tile(col, row).value() == lower.value()) {
                        board.move(col, row, lower);
                        score += lower.value() * 2;
                        break;
                    }
                    else if (board.tile(col, row) != null && lower != null && board.tile(col, row).value() != lower.value()) {
                        board.move(col, row - 1, lower);
                        break;
                    }
                    lowerRow--;
                }
